...
from quantize.omni_norm import OmniQwenRMSNorm
...
class QuantQwenAttention(nn.Module):
    ...
    def forward(
        ...
        if past_key_value is not None:
            kv_seq_len += past_key_value.get_usable_length(kv_seq_len, self.layer_idx) #kv_seq_len += past_key_value[0].shape[-2]
...
class QuantQwenDecoderLayer(nn.Module):
    def __init__(self, 
        ...
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = QuantQwenAttention(
        ...
        self.input_layernorm = OmniQwenRMSNorm(ori_layer.input_layernorm,eps=ori_layer.input_layernorm.variance_epsilon)
        self.post_attention_layernorm = OmniQwenRMSNorm(ori_layer.post_attention_layernorm,eps=ori_layer.post_attention_layernorm.variance_epsilon)
