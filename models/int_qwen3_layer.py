...
from quantize.omni_norm import OmniQwen3RMSNorm  
...
# ========== QuantQwen3Attention (核心修改：q_norm/k_norm) ==========
class QuantQwen3Attention(nn.Module):
    def __init__(self, 
        ...
        # 【新增】量化 q_norm / k_norm
        self.q_norm = OmniQwen3RMSNorm(org_module.q_norm, eps=org_module.q_norm.variance_epsilon)
        self.k_norm = OmniQwen3RMSNorm(org_module.k_norm, eps=org_module.k_norm.variance_epsilon)
        self.sliding_window = config.sliding_window if config.layer_types[self.layer_idx] == "sliding_attention" else None
        
        # MatMul 量化 (与 Qwen2 一致)
...
# ========== QuantQwen3DecoderLayer (整合 + LET 参数注册) ==========
class QuantQwen3DecoderLayer(nn.Module):
    def __init__(self, 
        ...
        # 包装 RMSNorm (input/post_attention)
        self.input_layernorm = OmniQwen3RMSNorm(ori_layer.input_layernorm, 
                                                eps=ori_layer.input_layernorm.variance_epsilon)
        self.post_attention_layernorm = OmniQwen3RMSNorm(ori_layer.post_attention_layernorm, 
                                                         eps=ori_layer.post_attention_layernorm.variance_epsilon)
        self.attention_type = ori_layer.attention_type
