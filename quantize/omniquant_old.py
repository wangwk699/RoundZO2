            ...
            with torch.no_grad():
                with torch.amp.autocast('cuda'):
                    for j in range(args.nsamples):
                        if is_qwen:
                            fp_inps[j] = qlayer(fp_inps[j].unsqueeze(0), attention_mask=attention_mask,position_embeddings=position_embeddings)
                            if args.aug_loss:
                                fp_inps_2[j] = qlayer(quant_inps[j].unsqueeze(0), attention_mask=attention_mask,position_embeddings=position_embeddings)[0]
                        else:
        ...
        if args.epochs > 0:
            ...
            for epochs in range(args.epochs):
                ...
                for j in range(args.nsamples//args.batch_size):    
                    ...
                    with traincast():
                        if is_qwen:
                            smooth_and_quant_temporary(qlayer, args, is_qwen) # 若禁用let，则这一步是只对权重进行量化
                            quant_out = qlayer(quant_inps[index:index+args.batch_size,], attention_mask=attention_mask_batch,position_embeddings=position_embeddings)
                        else:
        ...
        if args.epochs>0:
            ...
            with torch.no_grad():
                ...
                with traincast():
                    for j in range(args.nsamples):
                        if is_qwen:   
                            quant_inps[j] = qlayer(quant_inps[j].unsqueeze(0), attention_mask=attention_mask,position_embeddings=position_embeddings)
                        else:
