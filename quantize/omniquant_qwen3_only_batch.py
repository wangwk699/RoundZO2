...
# [QWEN3] 构造 per-layer causal mask mapping
...
def omniquant(
    ...
    attention_mask_mapping_batch = None
    ...
    class Catcher(nn.Module):
        def __init__(self, module):
            ...
            # 让 Qwen3Model 在访问 decoder_layer.attention_type 时不报错
        ...
        def forward(self, inp, **kwargs):
            ...
            if self.is_llama or self.is_qwen2:
                cache["attention_mask"] = kwargs["attention_mask"]
                cache["position_ids"] = kwargs["position_ids"]

            elif self.is_qwen3:
                # Qwen3 不依赖单一 attention_mask，而是每层按 attention_type 取 mapping 中一项
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            try:
                if is_qwen3:
                    # -----------------------------
                    # [QWEN3] 仿照 HF Qwen3Model.forward()
                    # 先统一构造 position_ids / position_embeddings / mask mapping
                    # -----------------------------
                    ...
                    # 这里传一个 mapping 进去，Qwen3Model.forward 会按 decoder_layer.attention_type 取用
                    model(
                        input_ids=input_ids,
    ...
    if is_llama or is_qwen2:
        ...
        attention_mask_mapping_batch = {}
        for k, v in attention_mask_mapping.items():
            if v is None:
                attention_mask_mapping_batch[k] = None
            else:
                attention_mask_mapping_batch[k] = (
                    v.repeat(args.batch_size, 1, 1, 1)
                    if args.deactive_amp else
                    v.repeat(args.batch_size, 1, 1, 1).float()
                )
    ...
    for i in range(len(layers)):
        ...
        # -----------------------------
        # [QWEN3] 为当前层选择 attention mask
        # -----------------------------
        if is_qwen3:
            layer_attention_mask = attention_mask_mapping[qlayer.attention_type]
            layer_attention_mask_batch = attention_mask_mapping_batch[qlayer.attention_type]
        else:
        ...
        if args.let:
            qlayer.register_parameter(
                "qkt_smooth_scale",
                torch.nn.Parameter(torch.ones(layer.self_attn.q_proj.out_features, device=dev, dtype=dtype))
            )
            ...
            for name, module in named_linears.items():
                ...
                if args.wbits == 3:
                    q_linear = qlinear_cuda.QuantLinear(
                        args.wbits, group_size, module.in_features, module.out_features, module.bias is not None
                    )
                else:
